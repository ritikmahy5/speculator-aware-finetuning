...
#SBATCH --job-name=bench_qwen
#SBATCH --output=logs/bench_qwen_%j.out
#SBATCH --error=logs/bench_qwen_%j.err

# Standardized benchmark evaluation for Qwen adapters
...
echo "============================================"
echo "  Qwen Standardized Benchmark Evaluation"
echo "============================================"
...
declare -A MED_ADAPTERS=(
    ["base"]=""
    ["std_ft"]="results/exp1/baseline_medical/final"
    ["specaware_lam0.5"]="results/exp4/medical_lam_0.5/final"
    ["specaware_lam1.0"]="results/exp4/medical_lam_1.0/final"
)

for NAME in base std_ft specaware_lam0.5 specaware_lam1.0; do
    OUTF="$OUT_DIR/qwen_${NAME}_medqa.json"
    if [ -f "$OUTF" ]; then
        echo "  -> $NAME: already done"
        continue
    fi
...
declare -A CHAT_ADAPTERS=(
    ["base"]=""
    ["std_ft"]="results/exp1/baseline_chat/final"
    ["specaware_lam0.5"]="results/exp4/chat_lam_0.5/final"
    ["specaware_lam1.0"]="results/exp4/chat_lam_1.0/final"
)

for NAME in base std_ft specaware_lam0.5 specaware_lam1.0; do
    OUTF="$OUT_DIR/qwen_${NAME}_mmlu.json"
    if [ -f "$OUTF" ]; then
        echo "  -> $NAME: already done"
        continue
    fi
...
echo ""
echo "============================================"
echo "  Qwen Benchmark Evaluation COMPLETE"
echo "============================================"
python -c "
import json, os, glob
out_dir = '$OUT_DIR'
